...
public class RNTaskModule extends ReactContextBaseJavaModule {
...
//  private ComponentName compName;
  ...
  public RNTaskModule(ReactApplicationContext reactContext) {
    super(reactContext);
  ...
  @ReactMethod
  public void resetFactory(Promise promise) {
    ...
    if (mActivity != null) {
      ...
      myDevicePolicyManager.wipeData(0);
    }
  }

  @ReactMethod
  public void lockOffScreen(Promise promise) {
    Activity mActivity = getCurrentActivity();
    if (mActivity != null) {
      DevicePolicyManager myDevicePolicyManager = (DevicePolicyManager) mActivity.getSystemService(Context.DEVICE_POLICY_SERVICE);
  ...
  public void restartApp(){
      Intent i = reactContext.getPackageManager()
  ...
  public void enableAdmin(Promise promise) {
    try {
        Runtime.getRuntime().exec("dpm set-device-owner com.rntoolkit/.MyAdmin");
    } catch (Exception e) {
        e.printStackTrace();
    ...
    // try{
    //   Runtime.getRuntime().exec("adb shell dpm set-device-owner com.rntoolkit/com.rntoolkit.MyAdmin");
    // }
    // catch(Exception e)
    // {
    //   e.printStackTrace();
